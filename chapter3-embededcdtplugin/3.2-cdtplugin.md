...
结合 MANIFEST.MF 文档的错误提示，发现 Dependencies Required Plug-ins 中有三个包报错。对照一下版本信息，发现之前按照 eclipse-embed-cdt/eclipse-plugins 工程需要导入的 CDT 9.2.1 版本的相关插件或者Feature的版本 与 Dependencies Required Plug-ins 中有三个包报错提示的版本不一致。
...
1. eclipese版本：[Eclipse IDE for RCP and RAP Developers](https://www.eclipse.org/downloads/packages/release/2024-09/r/eclipse-ide-rcp-and-rap-developers)  202409R
   ...
   >
2. eclipse安装的cdt插件版本：这里不同的仓库要求的版本不一样，一定要注意区分开
...
3. 关于 Java 环境：Eclipse 自身运行所需的JVM环境，与 Eclipse 中的 project JVM 运行环境是不同的。也是分开指定的。
   ...
   - [eclipse-embed-cdt](https://github.com/eclipse-embed-cdt/eclipse-plugins) 工程：根据仓库README文档中的环境描述信息。要求是 JavaSE-11 or later;
     看项目属性：看起来不是一个Java项目，所以没有指定 Java 版本。
     ![1731566601912](image/1731566601912.png)
     看Eclipse配置，java采用的是Eclipse默认的 jre：
     ![1731566446194](image/1731566446194.png)
